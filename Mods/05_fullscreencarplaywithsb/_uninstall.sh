...
echo "Starting uninstallation of Q3 Fullscreen CarPlay and AndroidAuto (with status bar) patch..."
...
echo "Disabling fullscreen AA Carplay (with status bar)..."

if [[ -e "/mnt/app/eso/hmi/lsd/jars" ]]; then
    if [[ -f "/mnt/app/eso/hmi/lsd/jars/FullScreenAACPCL_withsb.jar" ]]; then
        echo "Deleting jar..."
        rm -f /mnt/app/eso/hmi/lsd/jars/FullScreenAACPCL_withsb.jar
        echo "Done."
    else
        echo "Error: cannot find /mnt/app/eso/hmi/lsd/jars/FullScreenAACPCL_withsb.jar"
    fi
else
