...
echo "Starting installation of Q3 Fullscreen CarPlay and AndroidAuto (with status bar) patch..."
...
echo "Enabling fullscreen AA Carplay (with status bar)..."

if [[ -e "/mnt/app/eso/hmi/lsd/jars" ]]; then
    if [[ -f "$srcPath/Storage/jars/FullScreenAACPCL_withsb.jar" ]]; then
        echo "Copy jar..."
        cp -Vf $srcPath/Storage/jars/FullScreenAACPCL_withsb.jar /mnt/app/eso/hmi/lsd/jars/FullScreenAACPCL_withsb.jar
        echo "Done."
    else
        echo "Error: cannot find $dstPath/Storage/jars/FullScreenAACPCL_withsb.jar"
    fi
else
