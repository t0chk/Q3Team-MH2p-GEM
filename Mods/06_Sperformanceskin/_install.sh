...
echo "Starting installation of Q3 configurable S Performance skin..."
...
if [[ -e "/mnt/app/eso/hmi/lsd/jars" ]]; then
    if [[ -f "$srcPath/Storage/jars/CombiSkinsSPerfomance.jar" ]]; then
        echo "Copy jar..."
        cp -Vf $srcPath/Storage/jars/CombiSkinsSPerfomance.jar /mnt/app/eso/hmi/lsd/jars/CombiSkins.jar
        echo "Done."
    else
        echo "Error: cannot find $srcPath/Storage/jars/CombiSkinsSPerfomance.jar"
    fi
else
